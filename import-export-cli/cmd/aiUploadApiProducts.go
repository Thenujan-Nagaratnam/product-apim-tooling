...
var UploadAPIProductsCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		executeUploadAPIProductsCmd(cred, token, endpoint)
	},
}

// Do operations to upload APIs to the vector database
func executeUploadAPIProductsCmd(credential credentials.Credential, token, endpoint string) {
	accessToken, preCommandErr := credentials.GetOAuthAccessToken(credential, CmdUploadEnvironment)
	if preCommandErr != nil {
		utils.HandleErrorAndExit("Error while getting access token", preCommandErr)
	}
	impl.UploadAPIs(credential, accessToken, CmdUploadEnvironment, token, endpoint, uploadAll, true)
}
